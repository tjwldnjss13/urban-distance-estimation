...
from models.model2.conv import Conv, UpconvBilinear, ResidualBlock
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.upconv2 = nn.Sequential(
            UpconvBilinear(256, 64),
            *[ResidualBlock(64, 64) for _ in range(2)]
        )
        self.upconv1 = nn.Sequential(
            UpconvBilinear(64, 32),
            *[ResidualBlock(32, 32) for _ in range(2)]
        )

        self.iconv5 = Conv(1024+1024, 1024)
        self.iconv4 = Conv(512+512, 512)
        self.iconv3 = Conv(256+256+2, 256)
        self.iconv2 = Conv(64+64+2, 64)
        self.iconv1 = Conv(32+2, 32)

        self.disp4 = DisparityPrediction(512)
        self.disp3 = DisparityPrediction(256)
    ...
    def forward(self, skip1, skip2, skip3, skip4, x):
        up5 = self.upconv5(x)
...
if __name__ == '__main__':
    ...
    model = Decoder((8, 16)).cuda()
    skip1 = torch.ones(2, 64, 128, 256).cuda()
    skip2 = torch.ones(2, 256, 64, 128).cuda()
    skip3 = torch.ones(2, 512, 32, 64).cuda()
    skip4 = torch.ones(2, 1024, 16, 32).cuda()
    x = torch.ones(2, 2048, 8, 16).cuda()

    out = model(skip1, skip2, skip3, skip4, x)
