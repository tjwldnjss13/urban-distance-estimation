...
def main(args):
    ...
    model = ObjectDepthNet3(in_size, num_classes).to(device)
    ...
    for e in range(epoch):
        ...
        for fold, (train_idx, val_idx) in enumerate(kfold.split(dset)):
            ...
            train_loss_fold = 0
            train_loss_detector_fold = 0
            train_loss_depth_fold = 0
            train_iou_fold = 0

            val_loss_fold = 0
            val_loss_detector_fold = 0
            val_loss_depth_fold = 0
            val_iou_fold = 0
            ...
            for i, ann in enumerate(train_loader):
                ...
                imgl = make_batch(ann, 'imgl').to(device)
                imgr = make_batch(ann, 'imgr').to(device)
                ...
                loss, loss_detector, loss_depth, iou = model.loss(pred_detector, tar_detector, imgl, imgr, pred_disp)

                loss = loss.detach().cpu().item()

                train_loss_fold += loss
                train_loss_detector_fold += loss_detector
                train_loss_depth_fold += loss_depth
                train_iou_fold += iou
                ...
                print(f'<loss_detector> {loss_detector:.4f} ({train_loss_detector_fold/num_batch:.4f})  ', end='')
                print(f'<loss_depth> {loss_depth:.4f} ({train_loss_depth_fold/num_batch:.4f})  ', end='')
            ...
            train_loss_detector_fold /= num_batch
            train_loss_depth_fold /= num_batch
            ...
            print(f'<loss_detector> {train_loss_detector_fold:.4f}  ', end='')
            print(f'<loss_depth> {train_loss_depth_fold:.4f}  ', end='')
            ...
            for i, ann in enumerate(val_loader):
                ...
                val_loss_fold += loss
                val_loss_detector_fold += loss_detector
                val_loss_depth_fold += loss_depth
                val_iou_fold += iou
            ...
            val_loss_detector_fold /= num_batch
            val_loss_depth_fold /= num_batch
            val_iou_fold /= num_batch

            val_loss += val_loss_fold
            val_loss_detector += val_loss_detector_fold
            val_loss_depth += val_loss_depth_fold
            ...
            print('\t\t(valid) - ', end='')
            print(f'<loss> {val_loss_fold:.4f}  ', end='')
            print(f'<loss_detector> {val_loss_detector_fold:.4f}  ', end='')
            print(f'<loss_depth> {val_loss_depth_fold:.4f}  ', end='')
            print(f'<iou> {val_iou_fold:.4f}  ', end='')
            print(f'<time> {int(h):02d}:{int(m):02d}:{int(s):02d}')
        ...
        val_loss_detector /= args.num_k_fold
        val_loss_depth /= args.num_k_fold
        ...
        print(f'<loss_detector> {val_loss_detector:.4f}  ', end='')
        print(f'<loss_depth> {val_loss_depth:.4f}  ', end='')
